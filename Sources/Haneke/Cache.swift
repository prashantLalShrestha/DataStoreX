...
extension HanekeGlobals {
    ...
    public struct Cache {
        ...
        public static let SessionExpirableFormatName = "session_expirable"
...
open class Cache<T: DataConvertible> where T.Result == T, T : DataRepresentable {
    ...
    public init(name: String) {
        ...
        let originalFormat = Format<T>(name: HanekeGlobals.Cache.OriginalFormatName, expiry: .never)
    ...
    open func set(value: T, key: String, formatName: String = HanekeGlobals.Cache.OriginalFormatName, success succeed: ((T) -> ())? = nil) {
        if let (format, memoryCache, diskCache) = self.formats[formatName] {
            self.format(value: value, format: format) { formattedValue in
                let wrapper = ObjectWrapper(value: formattedValue, expiry: format.expiry)
                ...
                diskCache.updateExpiryDate(self.dataFromValue(formattedValue, format: format), key: key, date: format.expiry.date)
    ...
    @discardableResult open func fetch(key: String, formatName: String = HanekeGlobals.Cache.OriginalFormatName, failure fail : Fetch<T>.Failer? = nil, success succeed : Fetch<T>.Succeeder? = nil) -> Fetch<T> {
        ...
        if let (_, memoryCache, diskCache) = self.formats[formatName] {
            if let wrapper = memoryCache.object(forKey: key as AnyObject) as? ObjectWrapper, let result = wrapper.hnk_value as? T {
                if wrapper.expiry.date.inThePast == false {
                    fetch.succeed(result)
                } else {
                    fetch.fail()
                }
//                diskCache.updateExpiryDate(self.dataFromValue(result, format: format), key: key)
                return fetch
            }
    ...
    open func remove(formatName: String = HanekeGlobals.Cache.OriginalFormatName) {
        if let (_, memoryCache, diskCache) = self.formats[formatName] {
    ...
    fileprivate func fetchFromDiskCache(_ diskCache : DiskCache, key: String, memoryCache : NSCache<AnyObject, AnyObject>, failure fail : ((Error?) -> ())?, success succeed : @escaping (T, Expiry) -> ()) {
            ...
            DispatchQueue.global(qos: DispatchQoS.QoSClass.default).async(execute: {
                ...
                if let value = value {
                    ...
                    DispatchQueue.main.async(execute: {
                        let expiry = expiryDate.map({ Expiry.date($0) }) ?? .never
                        succeed(descompressedValue, expiry)
                        let wrapper = ObjectWrapper(value: descompressedValue, expiry: expiry)
                        memoryCache.setObject(wrapper, forKey: key as AnyObject)
                    })
                }
            })
